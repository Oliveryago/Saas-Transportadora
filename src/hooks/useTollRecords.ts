...
export function useTollRecords(vehicleId?: string, dateFilter?: DateFilter | null) {
    ...
    async function addRecord(record: Omit<TollRecord, "id" | "created_at" | "updated_at">) {
    ...
    async function updateRecord(id: string, updates: Partial<TollRecord>) {
        const { data, error: err } = await supabase
            .from("toll_records")
            .update(updates)
            .eq("id", id)
    ...
    async function deleteRecord(id: string) {
        const { error: err } = await supabase.from("toll_records").delete().eq("id", id);
        if (err) throw err;
