...
import type { WashingRecord } from "../types";
import { resolveDateFilter } from "./useDateFilter";
import type { DateFilter } from "../types";

export function useWashingRecords(vehicleId?: string, dateFilter?: DateFilter | null) {
    ...
    async function addRecord(record: Omit<WashingRecord, "id" | "created_at" | "updated_at">) {
    ...
    async function updateRecord(id: string, updates: Partial<WashingRecord>) {
        const { data, error: err } = await supabase
            .from("washing_records")
            .update(updates)
            .eq("id", id)
    ...
    async function deleteRecord(id: string) {
        const { error: err } = await supabase.from("washing_records").delete().eq("id", id);
        if (err) throw err;
