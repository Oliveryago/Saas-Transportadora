...
import { normalizePlan, type SubscriptionPlan } from "../config/planPermissions";
...
const PLAN_CONFIG: Record<SubscriptionPlan, { label: string; color: string; bg: string; border: string }> = {
    free: { label: "Free", color: "text-gray-700", bg: "bg-gray-100", border: "border-gray-400" },
    basic: { label: "Basic", color: "text-indigo-700", bg: "bg-indigo-50", border: "border-indigo-500" },
    premium: { label: "Premium", color: "text-amber-800", bg: "bg-amber-50", border: "border-amber-500" },
};

export function SuperAdmin() {
    const { user, signOut, isSuperAdmin, impersonateTenant, impersonatedTenant, isImpersonating } = useAuth();
    ...
    const [newCompanyPlan, setNewCompanyPlan] = useState<SubscriptionPlan>("basic");
    ...
    async function handleChangePlan(tenantId: string, newPlan: SubscriptionPlan) {
        setPlanMenuId(null);
        const { error } = await supabase.from("tenants").update({ subscription_plan: newPlan }).eq("id", tenantId);
        if (!error) {
            setTenants(prev => prev.map(t => t.id === tenantId ? { ...t, subscription_plan: newPlan } : t));
            if (selectedTenant?.id === tenantId) setSelectedTenant(prev => prev ? { ...prev, subscription_plan: newPlan } : null);
            if (isImpersonating && impersonatedTenant?.id === tenantId) {
                impersonateTenant({ ...impersonatedTenant, subscription_plan: newPlan });
            }
        }
    }
    ...
    async function handleCreateCompany(e: React.FormEvent) {
        ...
        try {
            const { data: tenant, error: tenantError } = await supabase
                .from("tenants")
                .insert([{ name: companyName, subscription_plan: newCompanyPlan, status: newCompanyStatus }])
                .select().single();
                ...
                {!selectedTenant ? (
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                >
                                                    ...
                                                    <div className="flex gap-4 text-sm text-gray-600">
                                                        ...
                                                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${PLAN_CONFIG[normalizePlan(tenant.subscription_plan)].bg} ${PLAN_CONFIG[normalizePlan(tenant.subscription_plan)].color}`}>
                                                            {PLAN_CONFIG[normalizePlan(tenant.subscription_plan)].label}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 ml-4" onClick={e => e.stopPropagation()}>
                                                    ...
                                                    <div className="relative">
                                                        <button
                                                            onClick={() => {
                                                                setPlanMenuId(null);
                                                                setStatusMenuId(statusMenuId === tenant.id ? null : tenant.id);
                                                            }}
                                                    ...
                                                    <div className="relative">
                                                        <button
                                                            onClick={() => {
                                                                setStatusMenuId(null);
                                                                setPlanMenuId(planMenuId === tenant.id ? null : tenant.id);
                                                            }}
                                                            className="flex items-center gap-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors"
                                                        >
                                                            Alterar Plano <ChevronDown className="w-3.5 h-3.5" />
                                                        </button>
                                                        {planMenuId === tenant.id && (
                                                            <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 w-40">
                                                                {(Object.entries(PLAN_CONFIG) as [SubscriptionPlan, typeof PLAN_CONFIG["free"]][]).map(([key, cfg]) => (
                                                                    <button
                                                                        key={key}
                                                                        onClick={() => handleChangePlan(tenant.id, key)}
                                                                        className={`flex items-center w-full px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors ${normalizePlan(tenant.subscription_plan) === key ? "font-semibold" : ""} ${cfg.color}`}
                                                                    >
                                                                        {cfg.label}
                                                                        {normalizePlan(tenant.subscription_plan) === key && <CheckCircle className="w-3.5 h-3.5 ml-auto" />}
                                                                    </button>
                                                                ))}
                                                            </div>
                                                        )}
                    ...
                    <>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="relative" onClick={e => e.stopPropagation()}>
                                    <button
                                        onClick={() => {
                                            setPlanMenuId(null);
                                            setStatusMenuId(statusMenuId === selectedTenant.id ? null : selectedTenant.id);
                                        }}
                                ...
                                <div className="relative" onClick={e => e.stopPropagation()}>
                                    <button
                                        onClick={() => {
                                            setStatusMenuId(null);
                                            setPlanMenuId(planMenuId === selectedTenant.id ? null : selectedTenant.id);
                                        }}
                                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium border-2 ${PLAN_CONFIG[normalizePlan(selectedTenant.subscription_plan)].bg} ${PLAN_CONFIG[normalizePlan(selectedTenant.subscription_plan)].color} ${PLAN_CONFIG[normalizePlan(selectedTenant.subscription_plan)].border}`}
                                    >
                                        {PLAN_CONFIG[normalizePlan(selectedTenant.subscription_plan)].label}
                                        <ChevronDown className="w-3.5 h-3.5" />
                                    </button>
                                    {planMenuId === selectedTenant.id && (
                                        <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 w-40">
                                            {(Object.entries(PLAN_CONFIG) as [SubscriptionPlan, typeof PLAN_CONFIG["free"]][]).map(([key, cfg]) => (
                                                <button
                                                    key={key}
                                                    onClick={() => handleChangePlan(selectedTenant.id, key)}
                                                    className={`flex items-center w-full px-4 py-2.5 text-sm hover:bg-gray-50 ${normalizePlan(selectedTenant.subscription_plan) === key ? "font-semibold" : ""} ${cfg.color}`}
                                                >
                                                    {cfg.label}
                                                    {normalizePlan(selectedTenant.subscription_plan) === key && <CheckCircle className="w-3.5 h-3.5 ml-auto" />}
                                                </button>
                                            ))}
                                        </div>
                                    )}
                        ...
                        <div className="bg-white rounded-lg shadow p-6 mb-6">
                            ...
                            <div className="flex gap-4 text-sm text-gray-600">
                                <span>Plano: <strong>{PLAN_CONFIG[normalizePlan(selectedTenant.subscription_plan)].label}</strong></span>
                        ...
                        <form onSubmit={handleCreateCompany} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(Object.entries(PLAN_CONFIG) as [SubscriptionPlan, typeof PLAN_CONFIG["free"]][]).map(([key, cfg]) => (
                                        <label key={key} className={`flex flex-col items-center gap-1 p-3 border-2 rounded-lg cursor-pointer transition-colors text-center ${newCompanyPlan === key ? `${cfg.border} ${cfg.bg}` : "border-gray-200 hover:border-gray-300"}`}>
                                            <input type="radio" name="company_plan" value={key} checked={newCompanyPlan === key}
                                                onChange={() => setNewCompanyPlan(key)} className="sr-only" />
